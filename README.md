...
## 原理科普：为什么有的软件能上外网，有的不能

「代理」不是一个开关，而是三条不同的路：**系统代理**、**虚拟网卡（TUN）**、**程序自带参数**。每个程序走哪条路，决定了它能不能上网——这也是「同一个电脑上浏览器打不开、某个软件却正常」的全部原因。

AG Bridge 走的是第三条：最干净、影响面最小，不动系统设置。

这套机制背后的通用知识整理成了一篇科普，不止适用于本项目，任何"为什么这个软件能上外网、那个不能"的问题都能套：

**[→ 为什么有的软件能上外网，有的不能](docs/为什么有的软件能上外网有的不能.md)**

里面包含三条路的对比、Electron 应用"跑起来了就补不上参数"的原因、系统代理崩溃导致全网断的兜底命令，以及五条自查命令。
...
- `docs/`：使用、设计和发布文档，以及一篇[网络代理科普](docs/为什么有的软件能上外网有的不能.md)。
